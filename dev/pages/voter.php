	...
	<div class="section row">
        <div class="col s12">
				...
				if(isset($_GET['success']))
				{
					...
					{
						?>
						<script>M.toast({html: "Votre récompense arrive, si vous n'avez pas vu de fenêtre s'ouvrir pour voter, la fenêtre à dû s'ouvrir derrière votre navigateur, validez le vote et profitez de votre récompense In-Game !"})</script>
					<?php
					}
					else
					{
						?>
						<script>M.toast({html: "Votre(vos) récompense(s) arrive(nt), profitez de votre(vos) récompense(s) In-Game !"})</script>
					<?php
					}
				}
				?>	
				<div style="display:none;" id="vote-success1" class="card green">Merci de votre vote ! Votre récompense arrive In-Game !</div>

<div class="card">
  <div class="panel-content">
	<div class="center">
	<h3>
	<?php echo $_Serveur_['General']['name']; ?> a besoin de vous !
	</h3>
	<p><strong>
	...
	if(isset($_Joueur_) AND  isset($_GET['player']) AND $_Joueur_['pseudo'] == $_GET['player'] )
		{
			...
			{
				echo '<div class="center"><center><ul style="list-style-position: inside; padding-left: 0px;">';
				$p=0;
				$list = array();
				$listNum = array();
				...
				echo "<a class='btn green darken-1' href='?action=recupVotesTemp' title='Récupérer mes récompenses'>Récupérer mes récompenses (Connectez-vous sur le serveur)</a></center></div>";
			}
			...
			<div class="tabbable">
				...
				<?php if(!isset($_GET['player'])) { ?>	
					<div class="well" style="background-color:white;padding:10px;">
						 <h4 class="center">Veuillez rentrer votre pseudo exact In-Game :</h4>
							<form  id="forme-vote" role="form" method="GET" action="index.php">
								<div style="width:60%;margin-right:20%; margin-left:20%;" class="center">
									<div class="row">
									<input type="text" style="display:none;" name="page" value="voter">
									 <input type="text" id="vote-pseudo" class="form-control col s12 m5" name="player" placeholder="Pseudo" <?php if(isset($_Joueur_)) { echo 'value="'.$_Joueur_['pseudo'].'"'; }?> required>
									<button class="btn green waves-effect" onclick="" type="submit">Suivant !</button>
								   </div>
								</div>
							</form>
					</div>
						...
						<div class="card-content">
							<div>
								<p class="center">
									...
									Bienvenue dans la catégorie de vote pour le serveur : <?=$lecture['Json'][$i]['nom'];?>
							...
							<?php  
								...
								if($data_count['count'] > 0)
								{
									...
									{ 
										...
										{
											echo '<button type="button" class="btn green" style="margin-top:5px; margin-right:5px;" disabled>'.GetTempsRestant($donnees['date_dernier'], $lectureVotes['temps'], $donnees).'</button>';
										}
										else if($action[0] != "jeton" || isset($_Joueur_))
										{
											if($lectureVotes['enligne'] == 1 && !$enligne) 
											{
												echo '<button type="button" class="btn red" style="margin-top:5px; margin-right:5px;" disabled>Vous devez être connecté sur le serveur pour pouvoir voter sur ce site.</button>';
										
											} else {
												echo '<a href="'.$liensVotes['lien'].'" style="margin-top:5px;" id="btn-lien-'.$id.'" target="_blank" onclick="document.getElementById(\'btn-lien-'.$id.'\').style.display=\'none\';document.getElementById(\'btn-verif-'.$id.'\').style.display=\'inline\';bouclevote('.$id.',\''.$pseudo.'\');" class="btn green" >'.$liensVotes['titre'].'</a>
													  <button id="btn-verif-'.$id.'" style="margin-top:5px; display:none;" type="button" class="btn red" disabled>Vérification en cours ...</button>
													  <button type="button" style="margin-top:5px; display:none;" id="btn-after-'.$id.'" class="btn gray" style="cursor: not-allowed;" disabled>'.TempsTotal($lectureVotes['temps']).'</button>
													';
											}
										} else {
											echo '<button type="button" class="btn red" style="margin-top:5px; margin-right:5px;" disabled>Vous devez être connecté sur le site pour pouvoir voter sur ce site.</button>';
